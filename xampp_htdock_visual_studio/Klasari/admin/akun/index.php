	...
	<div class="col-lg-4 col-xs-12">
		<div class="card card-primary">
			...
			<div class="card-body text-center">
				<img src="../assets/img/<?= $data['img'] ?>" class="img-rounded w-auto" style="width: 240px; height: 240px;">
				<h5 class="mt-3 text-primary text-uppercase"><?= $data['nama_pengguna'] ?></h5>	
				<?php 
					if($_SESSION['lvluser'] == 1) {
						echo "<h5 class='text-primary'>Admin</h5>";								
					} elseif($_SESSION['lvluser'] == 2) {
						echo "<h5 class='text-primary'>User</h5>";
					} 
				?>
			</div>
		</div>
 	...
 	if(isset($_POST['tambah'])) {
		...
		$file_tmp = $_FILES['file']['tmp_name'];
			...
			if(in_array($ekstensi, $ekstensi_boleh) === true) {
				if($ukuran < 2000000) {
					move_uploaded_file($file_tmp, '../assets/img/'. $gambar);
					$sql = mysqli_query($con, "INSERT INTO tbl_users VALUES ('', '$user', '$pass', '$pengguna', '$gambar', 1)");
